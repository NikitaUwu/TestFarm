...
Открыть [ферму](http://localhost:3000) и выбрать «Регистрация»: логин, почта и пароль (12–200 символов). Затем можно входить по логину или почте; регистр букв в них не важен. Каждый аккаунт получает отдельное пространство идей. Ранее созданные аккаунты доступны под логинами `admin` (прежний OWNER_PASSWORD) и `demo` (прежний REVIEWER_PASSWORD). Миграция сохраняет их данные и один раз переносит пароли в хеши в PostgreSQL. Без ключа ИИ сервис показывает «Не настроено»; исследование ожидает настройки.
