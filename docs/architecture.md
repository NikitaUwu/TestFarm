...
```text
Личный аккаунт → Web → API → PostgreSQL
...
Регистрация создаёт principal и accounts в одной транзакции. accounts хранит уникальные нормализованные логин/почту и хеш пароля; sessions связывает случайную сессию с principal, ideas.owner_id ограничивает личное пространство. Миграция 008 добавляет аккаунты к прежним ID без переноса или удаления идей. Клиент получает только id/username/email. При выходе компонент рабочего пространства размонтируется, поэтому следующий вход не наследует его состояние.
