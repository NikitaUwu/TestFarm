...
import AutonomousIdea from './AutonomousIdea';
...
function Workspace({session,onLogout}:{session:Account;onLogout:()=>void}){
  ...
  return <div className="app-shell">
    ...
    <main id="main" className="main">
      ...
      {view==='funnel'?<>
      ...
      </>:view==='plus'?<IdeaIntake onStarted={async(id)=>{setSelected(id);setView('activity');setDetail(await api('/ideas/'+id));await refresh();}}/>:view==='settings'?<Settings integrations={integrations} onRefresh={async()=>setIntegrations(await api('/integrations'))}/>:<>
        ...
        {!selected?<Empty title="Выберите идею" text="Откройте карточку из воронки или выберите её в списке."/>:!current?<p role="status">Загрузка карточки…</p>:current.runs[0]?.config_versions?.run_settings?.autonomous&&view!=='report'?<AutonomousIdea key={selected+':'+current.runs[0].id} idea={current} mode={view} refresh={refresh} onReport={()=>navigate('report')}/>:view==='card'?<IdeaEditor key={selected+':'+current.current_version} idea={current} onSaved={saved} onRefresh={refresh} onRun={()=>navigate('activity')} onDeleted={()=>{setSelected('');navigate('funnel');void refresh();}}/>:view==='activity'||view==='chart'?<Research idea={current} mode={view} refresh={refresh}/>:<Reports idea={current} refresh={refresh}/>}
      </>}
    </main>
  </div>;
}
